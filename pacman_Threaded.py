...
# Function defines ghost movement
    """
    Threaded version takes a ghost as an argument and does not loop
    through each ghost
    """
def move_ghost(ghost):
    ...
    """
    This function separates each ghost into their own threads, and calls the move_ghost function
    for each in turn before starting again at the top. 
    """
def move_ghosts():
    threads = []
    
    for ghost in ghosts:
        thread = threading.Thread(target=move_ghost, args=(ghost,))
